...
var LocalFileStore = class extends BaseStore {
	...
	*/
	async *yieldKeys(prefix) {
		const allKeys = (await fs.readdir(this.rootPath)).filter((file) => file.endsWith(".txt")).map((file) => file.replace(/\.txt$/, ""));
	...
	*/
	static async fromPath(rootPath) {
		...
		try {
			...
			await Promise.all(entries.filter((file) => file.endsWith(".tmp")).map((tempFile) => fs.unlink(path.join(rootPath, tempFile)).catch(() => {})));
		} catch {}
