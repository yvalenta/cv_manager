...
async function pull(ownerRepoCommit, options) {
	...
	if (options?.includeModel) {
		...
		if (Array.isArray(chatModelObject?.id)) {
			...
			if (modelName) {
				modelClass = await getChatModelByClassName(modelName, inferModelProviderFromNamespace(chatModelObject.id));
				if (!modelClass) console.warn(`Received unknown model name from prompt hub: "${modelName}"`);
			}
		}
	}
	return bindOutputSchema(await load(JSON.stringify(promptObject.manifest), options?.secrets, generateOptionalImportMap(modelClass), generateModelImportMap(modelClass), options?.secretsFromEnv));
}
//#endregion
