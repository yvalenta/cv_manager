	...
	constructor(options, defaultConfig) {
		...
		*/
		...
		const compileTransformers = [
			require_stream.createToolCallTransformer([]),
		...
		*/
		const graphDefaultConfig = require_utils.toGraphDefaultConfig(this.#defaultConfig);
	...
	*/
	...
	streamEvents(state, config, streamOptions) {
		...
		return (async () => {
			...
			const mergedConfig = (0, _langchain_core_runnables.mergeConfigs)(this.#defaultConfig, restConfig);
			const initializedState = await this.#initializeMiddlewareStates(state, mergedConfig);
