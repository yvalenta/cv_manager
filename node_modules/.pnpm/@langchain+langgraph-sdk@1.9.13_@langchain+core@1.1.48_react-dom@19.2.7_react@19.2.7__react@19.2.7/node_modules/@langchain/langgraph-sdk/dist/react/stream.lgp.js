...
import "../client.js";
import { userFacingInterruptsFromThreadTasks, userFacingInterruptsFromValuesArray } from "../ui/interrupts.js";
import { StreamError } from "../ui/errors.js";
import { getToolCallsWithResults } from "../utils/tools.js";
import { StreamManager } from "../ui/manager.js";
import { filterStream, findLast, onFinishRequiresThreadState, unique } from "../ui/utils.js";
...
function useStreamLGP(options) {
	...
	const messageMetadata = (() => {
		...
		return getMessages(historyValues).map((message, idx) => {
			...
			const firstSeenState = findLast(history.data ?? [], (state) => state.values != null && getMessages(state.values).map((m, idx) => m.id ?? idx).includes(messageId));
	...
	const submit = async (values, submitOptions) => {
		...
		const shouldRefetch = includeImplicitBranch || onFinishRequiresThreadState(options.onFinish);
		let callbackMeta;
		let rejoinKey;
		...
		await stream.start(async (signal) => {
			...
			async onSuccess() {
				...
				} else if (options.onFinish != null && !onFinishRequiresThreadState(options.onFinish)) options.onFinish(void 0, callbackMeta);
			},
			onError(error) {
	...
	const joinStream = async (runId, lastEventId, joinOptions) => {
		...
		const shouldRefetchJoin = historyLimit === true || typeof historyLimit === "number" || onFinishRequiresThreadState(options.onFinish);
		await stream.start(async (signal) => {
			...
			async onSuccess() {
				...
				if (!shouldRefetchJoin) {
					if (options.onFinish != null && !onFinishRequiresThreadState(options.onFinish)) options.onFinish(void 0, callbackMeta);
					return;
				}
	...
	useEffect(() => {
		flushPendingHeadlessToolInterrupts(values, options.tools, handledToolsRef.current, {
	...
	const streamHandle = {
		...
		get interrupts() {
			if (values != null && "__interrupt__" in values && Array.isArray(values.__interrupt__)) return userFacingInterruptsFromValuesArray(values.__interrupt__);
			if (stream.isLoading) return [];
			const taskInterrupts = userFacingInterruptsFromThreadTasks((branchContext.threadHead?.tasks ?? []).flatMap((t) => t.interrupts ?? []));
	...
	for (const key of [
		...
		if (descriptor?.get) Object.defineProperty(streamHandle, key, {
			...descriptor,
			enumerable: false
		});
	}
