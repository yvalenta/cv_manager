...
function useStreamCustom(options) {
	...
	useEffect(() => {
		flushPendingHeadlessToolInterrupts(stream.values, options.tools, handledToolsRef.current, {
	...
	return {
		...
		submit,
		get interrupts() {
			if (stream.values != null && "__interrupt__" in stream.values && Array.isArray(stream.values.__interrupt__)) return userFacingInterruptsFromValuesArray(stream.values.__interrupt__);
			return [];
		},
		get interrupt() {
