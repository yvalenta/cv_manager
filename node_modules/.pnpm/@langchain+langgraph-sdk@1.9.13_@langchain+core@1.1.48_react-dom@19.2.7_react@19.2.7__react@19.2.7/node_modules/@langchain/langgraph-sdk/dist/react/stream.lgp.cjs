...
const require_index = require("../client/index.cjs");
require("../client.cjs");
...
function getFetchHistoryKey(client, threadId, limit) {
	return [
		require_index.getClientConfigHash(client),
		threadId,
		limit
	].join(":");
}
...
function useStreamLGP(options) {
	...
	const client = (0, react.useMemo)(() => options.client ?? new require_index.Client({
	...
	(0, react.useEffect)(() => {
		require_headless_tools.flushPendingHeadlessToolInterrupts(values, options.tools, handledToolsRef.current, {
	...
	const streamHandle = {
		...
		get interrupts() {
			if (values != null && "__interrupt__" in values && Array.isArray(values.__interrupt__)) return require_interrupts.userFacingInterruptsFromValuesArray(values.__interrupt__);
			if (stream.isLoading) return [];
			const taskInterrupts = require_interrupts.userFacingInterruptsFromThreadTasks((branchContext.threadHead?.tasks ?? []).flatMap((t) => t.interrupts ?? []));
