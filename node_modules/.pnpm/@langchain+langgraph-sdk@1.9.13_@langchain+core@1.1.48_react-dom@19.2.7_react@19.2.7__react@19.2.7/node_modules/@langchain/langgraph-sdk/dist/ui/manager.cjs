...
var StreamManager = class {
	...
	*/
	async fetchSubagentHistory(threads, threadId, options) {
		...
		*/
		const toolCallIdToNamespace = /* @__PURE__ */ new Map();
		try {
			/**
				...
				for (const task of tasks) {
					...
					*/
					...
					for (const msg of resultMessages) {
						const m = msg;
						if (m.type === "tool" && typeof m.tool_call_id === "string" && toFetch.some(([id]) => id === m.tool_call_id) && !toolCallIdToNamespace.has(m.tool_call_id)) toolCallIdToNamespace.set(m.tool_call_id, `${task.name}:${task.id}`);
					}
				}
			}
			/**
			...
			*/
			const hasUnmappedToolCalls = () => toFetch.some(([id]) => !toolCallIdToNamespace.has(id));
			if (hasUnmappedToolCalls()) for (const checkpoint of mainHistory) {
				...
				*/
				...
				for (let i = 0; i < sorted.length && i < subagentToolCalls.length; i += 1) {
					...
					const task = sorted[i];
					if (tc?.id && task.id && task.name && toFetch.some(([id]) => id === tc.id) && !toolCallIdToNamespace.has(tc.id)) toolCallIdToNamespace.set(tc.id, `${task.name}:${task.id}`);
				}
			}
		} catch {}
		/**
			...
			for await (const { event, data } of run) {
				...
				if (event === "values" || event.startsWith("values|")) if (namespace && require_subagents.isSubagentNamespace(namespace)) {
					...
					if (data && typeof data === "object" && "__interrupt__" in data) {
						...
						this.setStreamValues((prev) => {
							const prevInterrupts = prev?.__interrupt__;
							let interrupts = interruptData.__interrupt__;
							if (Array.isArray(interrupts)) {
								if (interrupts.length === 0) interrupts = [];
								else if (Array.isArray(prevInterrupts)) {
									const mergedInterrupts = [...prevInterrupts];
									const existingIds = new Set(prevInterrupts.map((i) => i.id).filter((id) => id != null));
					...
					} else this.setStreamValues((prev) => {
						if (prev && "__interrupt__" in prev && Array.isArray(prev.__interrupt__)) return {
							...data,
							__interrupt__: prev.__interrupt__
						};
						return data;
					});
				}
				if (this.matchEventType("messages", event, data)) {
					...
					const eventCheckpointNs = namespace && require_subagents.isSubagentNamespace(namespace) ? namespace : void 0;
					const rawCheckpointNs = metadata?.langgraph_checkpoint_ns || metadata?.checkpoint_ns;
					const checkpointNs = eventCheckpointNs ?? (typeof rawCheckpointNs === "string" ? rawCheckpointNs.split("|") : void 0);
					const isFromSubagent = require_subagents.isSubagentNamespace(checkpointNs);
					const toolCallId = isFromSubagent ? require_subagents.extractToolCallIdFromNamespace(checkpointNs) : void 0;
